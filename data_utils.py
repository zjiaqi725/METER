...
def load_dataset(args):
    ...
    if args.dataset == 'NSL':
        ...
        data = pd.read_csv('datasets/numenta/ambient_temperature_system_failure.csv')
        ...
        data = pd.read_csv('datasets/numenta/cpu_utilization_asg_misconfiguration.csv')
        ...
        data = pd.read_csv('datasets/numenta/machine_temperature_system_failure.csv')
        numeric = data["value"].values
        labels = data["label"].values
        X = shingle(numeric, 10) 
        numeric = torch.FloatTensor(np.transpose(X))
        ...
        data = pd.read_csv('datasets/numenta/nyc_taxi.csv')
        numeric = data["value"].values
        labels = data["label"].values
        X = shingle(numeric, 10) 
        numeric = torch.FloatTensor(np.transpose(X))
        ...
        data = pd.read_csv("datasets/ARCUS_DATA/"+args.dataset+".csv", dtype=np.float64, header=None)
